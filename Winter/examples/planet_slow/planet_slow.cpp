...
// Slow version without instancing of the planets demo
#include <array>
#include <algorithm>
#include <filesystem>
#include <fstream>
...
int main()
{
	...
	const std::string PATH_EXAMPLE = PathManager::getProjectPath() + "examples/planet_slow/";
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		for (unsigned int i = 0; i < amount; i++)
		{
			unlitShader.setMat4("model", glm::value_ptr(modelMatrices[i]));
			rockModel.draw(unlitShader);
		}
