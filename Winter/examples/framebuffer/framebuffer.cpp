...
int main()
{
	...
	const std::string PATH_EXAMPLE = PathManager::getProjectPath() + "examples/framebuffer/";

	const std::string PATH_VERTEX_SHADER = PATH_EXAMPLE + "basicVertexShader.glsl";
    const std::string PATH_FRAGMENT_SHADER = PATH_EXAMPLE + "basicFragmentShader.glsl";
    const std::string PATH_LIGHT_VERTEX_SHADER = PATH_EXAMPLE + "lightCubeVertexShader.glsl";
    const std::string PATH_LIGHT_FRAGMENT_SHADER = PATH_EXAMPLE + "lightCubeFragmentShader.glsl";
    const std::string PATH_SCREEN_VERTEX_SHADER = PATH_EXAMPLE + "screenVertexShader.glsl";
    const std::string PATH_SCREEN_FRAGMENT_SHADER = PATH_EXAMPLE + "screenFragmentShader.glsl";
    
    const std::string PATH_TEXTURE_CONTAINER = PathManager::getTexturesPath() + "container.jpg";
    const std::string PATH_TEXTURE_SMILE = PathManager::getTexturesPath() + "awesomeface.png";
    ...
    const std::string PATH_TEXTURE_MATRIX = PathManager::getTexturesPath() + "matrix.jpg";
    const std::string PATH_TEXTURE_WINDOW = PathManager::getTexturesPath() + "window.png";
    const std::string PATH_TEXTURE_GRASS = PathManager::getTexturesPath() + "grass.png";
    ...
    unsigned int smileyTexture = Texture::loadTexture(PATH_TEXTURE_SMILE, false);
	unsigned int container2Texture = Texture::loadTexture(PATH_TEXTURE_CONTAINER2, false);
    unsigned int container2Specular = Texture::loadTexture(PATH_TEXTURE_CONTAINER2_SPECULAR, false);
    unsigned int matrixTexture = Texture::loadTexture(PATH_TEXTURE_MATRIX, false);
    unsigned int grassTexture = Texture::loadTexture(PATH_TEXTURE_GRASS, false, GL_CLAMP_TO_EDGE);
    unsigned int windowTexture = Texture::loadTexture(PATH_TEXTURE_WINDOW, false);
    ...
    Mesh grassQuad = quad;
	grassQuad.AddTexture(Texture(grassTexture, "texture_diffuse", PATH_TEXTURE_GRASS));

	Mesh windowQuad = quad;
	windowQuad.AddTexture(Texture(windowTexture, "texture_diffuse", PATH_TEXTURE_WINDOW));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            rotate = glm::rotate(rotate, glm::radians(180.0f), camera.GetUp());
            glm::vec3 res = glm::vec3(rotate * glm::vec4(camera.GetFront(), 1.0f));
            camera.SetFront(res);
    ...
    glDeleteTextures(1, &matrixTexture);
